...
export const is_IntermediaryCalculation = validate45;
function validate45(data: unknown, { instancePath = "", parentData, parentDataProperty, rootData = data, dynamicAnchors = {} }: Partial<Omit<Exclude<Parameters<ValidateFunction>[1], undefined>, "rootData"> & {
    rootData: unknown;
}> = {}): data is stub_IntermediaryCalculation {
    let vErrors: ErrorObject[] = [];
    let errors = 0;
    const evaluated0 = ajv_utiltiies__definitely_evaluated(validate45);
    if (evaluated0?.dynamicProps) {
        evaluated0.props = undefined;
    }
    if (evaluated0?.dynamicItems) {
    ...
    (validate45 as Is).errors = vErrors.length ? vErrors : null;
    return errors === 0;
}
(validate45 as Is).evaluated = { "props": true, "dynamicProps": false, "dynamicItems": false };
export const is_CanConvertTypeJson = validate48;
function validate48(data: unknown, { instancePath = "", parentData, parentDataProperty, rootData = data, dynamicAnchors = {} }: Partial<Omit<Exclude<Parameters<ValidateFunction>[1], undefined>, "rootData"> & {
    rootData: unknown;
}> = {}): data is stub_CanConvertJson {
    let props0: (true | {
        [key: string]: true;
    ...
    let vErrors: ErrorObject[] = [];
    let errors = 0;
    const evaluated0 = ajv_utiltiies__definitely_evaluated(validate48);
    if (evaluated0?.dynamicProps) {
        evaluated0.props = undefined;
    }
    if (evaluated0?.dynamicItems) {
    ...
    (validate48 as Is).errors = vErrors.length ? vErrors : null;
    evaluated0.props = props0;
    return errors === 0;
}
(validate48 as Is).evaluated = { "dynamicProps": true, "dynamicItems": false };
